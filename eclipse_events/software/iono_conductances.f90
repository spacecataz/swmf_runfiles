...
subroutine ionosphere_conductance(Sigma0, SigmaH, SigmaP, &
  ...
  real :: BufferCond_VII(2, IONO_nTheta, IONO_nPsi) = 0.0
  real :: tempHall, tempPed, tempLon, tempLat
  ...
  if (use_comp(UA_)) then
     ...
     if (iModel == 12) then
         ! Load conductances from file.
         ! Create time stamp for files:
         imin_floor = 10 * floor(Time_Array(5)/10.)
         ...
         write(*,*) "DEBUG: Opening file", 'IE/cond_'// NameHemShort // &
            '_t' // tstamp //'.dat'
     ...
     if ((iModel.ge.3) .and. (iModel.ne.10) .and. (iModel/=12)) then
