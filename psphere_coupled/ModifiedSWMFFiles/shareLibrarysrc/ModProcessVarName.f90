...
contains
  ...
  subroutine process_var_list(nVarName, NameVar_V,  &
    ...
    NAMELOOP: do iName = 1, nVarName
       ...
       if(.not. IsFoundVar) then
          write(*,*) 'ERROR: Var name not in dictionary: ',NameVarIn !search
  ...
  contains
    ...
    subroutine find_substance_replace_name
      ...
      !------------------------------------------------------------------------
      !write(*,*) "SubstanceStandardName_II: ", SubstanceStandardName_II
  ...
  subroutine create_standard_name
    ...
    !--------------------------------------------------------------------------
    !write(*,*) "create_standard_name"
    !write(*,*) "NameSubstance_I: ", NameSubstance_I
    !write(*,*) "NameSubstanceVar_I: ", NameSubstanceVar_I
    do iSubstance = 1, nSubstance
       do iVar = 1, nVarPerSubstance
