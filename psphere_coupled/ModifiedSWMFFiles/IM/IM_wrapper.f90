...
contains
  ...
  subroutine IM_put_from_gm_crcm(Buffer_IIV,BufferKp,BufferAe,iSizeIn,jSizeIn,nVarIn,&
    ...
    use ModCimiPlanet,ONLY: rEarth => re_m, NameVarCouple,nVarImToGm,&
         Sw_, H_, O_, He_, PsHp_, e_, UseRecirculatedPs
    ...
    !-------------------------------------------------------------------------
    call CON_set_do_test(NameSub, DoTest, DoTestMe)
    !write(*,*) "IM_wrapper:IM_put_from_gm_crcm:"
...
!       endif
       ...
       do iVarBuffer = 1,nVarBuffer
          ...
          case('HpSwPpar','hpswppar')
             ...
             iPpar=iPpar+1
          case('PsHpRho','pshprho')
             write(*,*) 'PsHpRho=',iVarBuffer
             DoFeedbackPs = .true.
             if (UseRecirculatedPs) then
                UseMultiRhoGm = .true.
                iBufferRho_I(PsHp_) = iVarBuffer+5
                iRho=iRho+1
             endif
          case('PsHpP','pshpp')
             write(*,*) 'PsHpP=',iVarBuffer
             if (UseRecirculatedPs) then
                UseMultiPGm = .true.
                iBufferP_I(PsHp_) = iVarBuffer+5
                iP = iP+1
             endif
          end select
       end do
    ...
    write(*,*) "Number of Densities (iRho)", iRho
    write(*,*) "Number of Pressures (iP)", iP
    DoneGmCoupling=.true.
    ...
    StateBmin_IIV(:,:,:) = Buffer_IIV(:,:,:) !search
    !StateBmin_IIV(:,:,6) =  2.5e-20 ! Hp Density      | Density and pressure being sent are in SI units. These values
    !StateBmin_IIV(:,:,8) =  1.2e-8  ! Hp Pressure     | Should yeild a Number density of 15 cm^-3 and Temp of
    !StateBmin_IIV(:,:,9) =  2.5e-20 ! PsHp Density    | 5 keV in both the solar wind fluid and recirc fluid.
    !StateBmin_IIV(:,:,10) = 1.2e-8  ! PsHp Pressure   | (within rounding error) Note- values produced might vary if
                                    !                 | using a CIMI restart file.
    !write(*,*) "StateBmin_IIV(:,:,6) [HpRho]: max, min"
    !write(*,*) maxval(StateBmin_IIV(:,:,6)), minval(StateBmin_IIV(:,:,6))
    !write(*,*) "StateBmin_IIV(:,:,8) [HpP]: max, min"
    !write(*,*) maxval(StateBmin_IIV(:,:,8)), minval(StateBmin_IIV(:,:,8))
    !write(*,*) "StateBmin_IIV(:,:,9) [PsHpRho]: max, min"
    !write(*,*) maxval(StateBmin_IIV(:,:,9)), minval(StateBmin_IIV(:,:,9))
    !write(*,*) "StateBmin_IIV(:,:,10) [PsHpP]: max, min"
    !write(*,*) maxval(StateBmin_IIV(:,:,10)), minval(StateBmin_IIV(:,:,10))
...
!    write(*,*) 'iLatMin',iLatMin,rBodyGm,StateBmin_IIV(iLatMin,1,iBufferRho_I(1)),StateBmin_IIV(iLatMin,1,iBufferRho_I(1))
  ...
  subroutine IM_get_for_gm(Buffer_IIV,iSizeIn,jSizeIn,nVar,NameVar)
    ...
    call split_string(NameVarCouple, NameVarCouple_V, &
         nStringOut=nVarCouple)
    !write(*,*) "IM_wrapper:IM_get_for_gm:NameVarCouple_V", NameVarCouple_V !search
    ...
    do iVarCimi = 1,nVarImToGm
       ...
       case('Rho')
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('Rho')" !sesarch
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('p')
          do i=1,iSize; do j=1,jSize
             if( i<iLatMin .or.  i > iba(j) ) then
                Buffer_IIV(i,j,iVarCimi) = -1.
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('p')" !sesarch
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
!          write(*,*) 'IM max min p',maxval(Buffer_IIV(:,:,iVarCimi)),minval(Buffer_IIV(:,:,iVarCimi))
       case('Ppar')
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('Ppar')" !sesarch
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
!          write(*,*) 'IM max min ppar',maxval(Buffer_IIV(:,:,iVarCimi)),minval(Buffer_IIV(:,:,iVarCimi))
       case('Pe')
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('Pe')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('HpRho')
          do i=1,iSize; do j=1,jSize
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('HpRho')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('HpP')
          do i=1,iSize; do j=1,jSize
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('HpP')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('HpPpar')
          do i=1,iSize; do j=1,jSize
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('HpPar`')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('OpRho')
           do i=1,iSize; do j=1,jSize
       ...
       case('PsHpRho', 'pshprho')
          do i=1,iSize; do j=1,jSize
             if( i<iLatMin .or.  i > iba(j) ) then
                ...
                Buffer_IIV(i,j,iVarCimi) = &
                     density (i,j)*cProtonMass !density is the plasmasphere density. It's allllllllllll good.
                     !PlasDensity_C (i,j)*cProtonMass
             end if
          enddo; enddo
        !write(*,*) "IM_wrapper:IM_get_for_gm:Case('PsHpRho')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) "      :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       case('PsHpP', 'pshpp')
          do i=1,iSize; do j=1,jSize
          ...
          enddo; enddo
          !write(*,*) "IM_wrapper:IM_get_for_gm:Case('PsHpP')" !search
          !write(*,*) "      :iVarCimi", iVarCimi
          !write(*,*) ":     :Buffer_IIV(:,:,iVarCimi)", Buffer_IIV(:,:,iVarCimi)
       end select
    end do
    !write(*,*) "IM_wrapper IM_getfor_gm" !search
    !write(*,*) "iSize, jSize, iLatMin", iSize, jSize, iLatMin
    !write(*,*) "iba", iba
    do i=1,iSize; do j=1,jSize
       if( i<iLatMin .or.  i > iba(j) ) then
          Buffer_IIV(i,j,iVarCimi) = -1.
