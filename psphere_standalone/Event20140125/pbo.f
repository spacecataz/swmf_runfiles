...
!     ARTIFICIAL INITIAL STRUCTURE:
           !write(*,*) vphicells(j), vrcells(i), dn, (sin(cDegToRad*vphicells(j)*6 ))**2, sin(cPi*vrcells(i)/2.)**2
           mgridden(i,j) = dn !+ dn/2 * sin(cDegToRad*vphicells(j)*6 ) *
!     *          sin(cpi*vrcells(i))
