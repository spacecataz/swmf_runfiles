...
SUBROUTINE THERMAL
  ...
  If (ithermfirst.eq.2) then
     If (itherminit.eq.1) then  ! Read initial plasmasphere from file
        ...
        do i=1, nrcells-1
           do j=1, nphicells
              ! Artificial initial conditions:
              !mgridden(i,j) = mgridden(i,j) + mgridden(i,j) * &
              !     sin(cDegToRad*vphicells(j)*6 ) * sin(2*cpi*vrcells(i))
              mgridden(i,j) = mgridden(i,j) * &
                   sin(cDegToRad*vphicells(j)*2 )**2 * sin(cpi*vrcells(i)/5.)**2
